#!/usr/bin/env python3
"""Facebook Auto-Post Skill - Automated Facebook Posting"""
...
class FacebookAutoPostSkill:
    """
    Agent skill for automated Facebook posting.
    
    Capabilities:
    - Auto-post to Facebook from Vault tasks
    - Session reuse via facebook_session.json
    ...
    def _load_posted_ids(self):
        """Load previously posted Facebook post IDs from tracking file"""
        ...
        if tracking_file.exists():
            ...
            BronzeLogger.log_skill_execution(
                self.logger, "FacebookAutoPostSkill", "load_posted_ids",
    ...
    def authenticate(self) -> bool:
        """
        Authenticate with Facebook using Playwright with session reuse.
        ...
        try:
            ...
            BronzeLogger.log_skill_execution(
                self.logger, "FacebookAutoPostSkill", "authenticate",
            ...
            if self.session_file.exists():
                print(f"[INFO] Loading saved Facebook session from {self.session_file}")
                try:
    ...
    def _check_if_logged_in(self) -> bool:
        ...
        try:
            ...
            # Check for post composer (only visible when logged in)
            composer = self.page.query_selector('[placeholder*="What\'s on your mind"]')
            if composer:
                return True
    ...
    def post_to_facebook(self, content: str, post_id: str) -> bool:
        ...
        """
        BronzeLogger.log_skill_execution(
            self.logger, "FacebookAutoPostSkill", "post_to_facebook",
        ...
        try:
            ...
            if not composer:
                BronzeLogger.log_skill_execution(
                    self.logger, "FacebookAutoPostSkill", "post_to_facebook",
                    "FAILED", "Post composer not found"
                )
                return False
            ...
            # Find textarea in opened composer
            textarea_selectors = [
    ...
    def post_from_vault_task(self, task_path: Path) -> bool:
        """
        Create Facebook post from Vault task
        ...
        """
        BronzeLogger.log_skill_execution(
            self.logger, "FacebookAutoPostSkill", "post_from_vault_task",
        ...
        try:
            ...
            if not post_content:
                BronzeLogger.log_skill_execution(
                    self.logger, "FacebookAutoPostSkill", "post_from_vault_task",
                    "FAILED", "Could not extract post content"
                )
                return False
            ...
            # Post to Facebook
            success = self.post_to_facebook(post_content, post_id)
    ...
    def _extract_post_content(self, task_content: str) -> Optional[str]:
        ...
        if "## Facebook Post" in task_content:
            lines = task_content.split('\n')
            post_lines = []
            in_section = False

            for line in lines:
                if line.strip() == "## Facebook Post":
                    in_section = True
                    continue
            ...
            content = '\n'.join(post_lines)
            return content[:63206] if content else None  # Facebook limit
        ...
        if "## Description" in task_content:
            ...
            content = '\n'.join(desc_lines)
            return content[:63206] if content else None
...
"""
            ...
            BronzeLogger.log_skill_execution(
                self.logger, "FacebookAutoPostSkill", "_log_post_to_vault",
    ...
    def close(self):
        ...
        if self.browser:
            try:
                self.browser.close()
                BronzeLogger.log_skill_execution(
                    self.logger, "FacebookAutoPostSkill", "close",
